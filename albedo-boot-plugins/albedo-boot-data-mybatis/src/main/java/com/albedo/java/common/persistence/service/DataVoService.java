...
public abstract class DataVoService<Repository extends BaseRepository<T, PK>,
        T extends IdEntity, PK extends Serializable, V extends DataEntityVo> extends DataService<Repository, T, PK> {
    ...
    public void save(V form) {
        T entity = null;
        try {
            entity = PublicUtil.isNotEmpty(form.getId()) ? repository.selectById(form.getId()) :
