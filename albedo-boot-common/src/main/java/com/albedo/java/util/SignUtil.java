...
import com.albedo.java.util.exception.MyException;
...
public class SignUtil {
     ...
     * @return 要请求的签名参数数组
     */
    public static Map<String, String> signMap(Map<String, String[]> sParaTemp,String secret) {
        //请求参数Map转换验证Map，并生成要请求的签名参数数组
        return sign(toVerifyMap(sParaTemp, false),secret);
    }
     ...
     * @return 要请求的签名参数数组
     */
    public static Map<String, String> sign(Map<String, String> sParaTemp,String secret) {
        //时间戳加入签名参数组中
        ...
        //生成签名结果
        String mysign = DigestUtils.md5Hex(getContentBytes(prestr + secret, INPUT_CHARSET));
        //签名结果加入请求提交参数组中
        sPara.put("sign", mysign);
        return sPara;
    }

    public static String getSignStr(Map<String, String> sParaTemp,String secret) {
        return sign(sParaTemp,secret).get("sign");
    }
     ...
     * @return 请求的签名参数字符串
     */
    public static String signStringMap(Map<String, String[]> sParaTemp,String secret) {
        //生成要请求的签名参数数组
        Map<String, String> sign = signMap(sParaTemp,secret);
        //生成要请求的签名参数字符串“参数=参数值”&链接
     ...
     * @return
     */
    public static String signString(Map<String, String> sParaTemp,String secret) {
        //生成要请求的签名参数数组
        Map<String, String> sign = sign(sParaTemp,secret);
        //生成要请求的签名参数字符串“参数=参数值”&链接
        return createLinkString(sign, true);
    }
     ...
     * @return 验证结果
     */
    public static boolean verifyMap(Map<String, String[]> paramsMap,String secret) throws Exception {
        //请求参数Map转换验证Map，并根据反馈回来的信息，生成签名结果
        return verify(toVerifyMap(paramsMap, false),secret);
    }
    ...
    public static boolean verify(Map<String, String> params,String secret) throws Exception {
        ...
        if (params.get("sign") != null) {
            ...
            throw new MyException("sign is null");
        }
        String timestamp = "";
        if (params.get("timestamp") != null) {
            timestamp = params.get("timestamp");
        }else {
            throw new MyException("timestamp is null");
        }
        //过滤空值、sign
        ...
        //获得签名验证结果
        String mysign = DigestUtils.md5Hex(getContentBytes(preSignStr + secret, INPUT_CHARSET));
        if (mysign.equals(sign)) {
            //是否超时
            long curr = System.currentTimeMillis();
            if ((curr - Long.valueOf(timestamp)) > TIME_OUT){
                logger.info("api is time out");
                throw new MyException("api is time out");
            }
            return true;
        } else {
            throw new MyException("sign is invalid");
        }
    }
